...
// Advent of Code 2024 day 5 part 2 solution
...
function main() {
    ...
    const outOfOrderUpdates = updates.filter(
        (update) => !isUpdateOrdered(orderingRules, update),
    );

    const orderedUpdates = putUpdatesInOrder(rules, outOfOrderUpdates);
...
/**
 * Returns a new array containing the given updates, but ordered according to
 * the given rules.
 * @param {Map<number, Array<number>>} rules The ordering rules from the input.
 * @param {Array<Array<number>>} updates The update from the input.
 * @returns {Array<Array<number>>} The in-order updates.
 */
function putUpdatesInOrder(rules, updates) {
    const result = [];
    ...
    for (const update of updates) {
        ...
        for (let i = 0; i < update.length; i++) {
            const pageNumber = update.at(i);

            if (!rules.has(pageNumber)) {
                break;
            }

            const pageNumbersThatShouldComeBeforeThisOne =
                rules.get(pageNumber);
